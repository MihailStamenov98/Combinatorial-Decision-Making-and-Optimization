...
from common_func import lessOrEq
...
def solveInstance(data: ReadData, minHeight: int, maxHeight: int):
    ...
    # symmetry = lessOrEq(blocksX, [w - blocksX[i] - widths[i]
    #                    for i in range(n)], n, max(maxHeight, w), "x")
    # symmetry += lessOrEq(blocksY, [l - blocksY[i] - heights[i]
    #                               for i in range(n)], n, max(maxHeight, w), "y")
#
    constraints = obj + yBounds + xBounds + noOverlap  # + symmetry
    return constraints
