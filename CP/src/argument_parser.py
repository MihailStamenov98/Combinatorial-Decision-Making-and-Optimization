...
class ReturnType:
    def __init__(self, args):
        ...
        if args.search is not None and args.restart is not None:
            ...
            filteredSolverStrategyes = list(
                filter(lambda x: (x[1] == args.restart), solverStrategyes))
        elif args.search is not None:
            filteredSolverStrategyes = list(
                filter(lambda x: (x[0] == args.search), solverStrategyes))
...
def parsArguments():
    ...
    parser.add_argument(
        "-g", "--gecode", help="Flag to use Gecoden", action='store_true')
